...
# proc 'SciterElementUnwrap' skipped - static inline procs cannot work with '--noHeader | -H'
# proc 'SciterElementWrap' skipped - static inline procs cannot work with '--noHeader | -H'
# proc 'SciterNodeUnwrap' skipped - static inline procs cannot work with '--noHeader | -H'
# proc 'SciterNodeWrap' skipped - static inline procs cannot work with '--noHeader | -H'
...
const
  ...
  STATE_LINK* = (0x00000001).ELEMENT_STATE_BITS ## ```
                                                ##   :link pseudo-class in CSS
                                                ## ```
  STATE_HOVER* = (0x00000002).ELEMENT_STATE_BITS ## ```
                                                 ##   :hover pseudo-class in CSS
                                                 ## ```
  ...
  STATE_CURRENT* = (0x00000020).ELEMENT_STATE_BITS ## ```
                                                   ##   :current, current (hot) item
                                                   ## ```
  STATE_CHECKED* = (0x00000040).ELEMENT_STATE_BITS ## ```
                                                   ##   :checked, element is checked (or selected)
                                                   ## ```
  STATE_DISABLED* = (0x00000080).ELEMENT_STATE_BITS ## ```
                                                    ##   :disabled, element is disabled
                                                    ## ```
  STATE_READONLY* = (0x00000100).ELEMENT_STATE_BITS ## ```
                                                    ##   :read-only, readonly input element
                                                    ## ```
  STATE_EXPANDED* = (0x00000200).ELEMENT_STATE_BITS ## ```
                                                    ##   :expanded, expanded state - nodes in tree view
                                                    ## ```
  STATE_COLLAPSED* = (0x00000400).ELEMENT_STATE_BITS ## ```
                                                     ##   :collapsed, collapsed state - nodes in tree view - mutually exclusive with
                                                     ## ```
  STATE_INCOMPLETE* = (0x00000800).ELEMENT_STATE_BITS ## ```
                                                      ##   :incomplete, fore (content) image was requested but not delivered
                                                      ## ```
  STATE_ANIMATING* = (0x00001000).ELEMENT_STATE_BITS ## ```
                                                     ##   :animating, is animating currently
                                                     ## ```
  STATE_FOCUSABLE* = (0x00002000).ELEMENT_STATE_BITS ## ```
                                                     ##   :focusable, will accept focus
                                                     ## ```
  STATE_ANCHOR* = (0x00004000).ELEMENT_STATE_BITS ## ```
                                                  ##   :anchor, anchor in selection (used with current in selects)
                                                  ## ```
  STATE_SYNTHETIC* = (0x00008000).ELEMENT_STATE_BITS ## ```
                                                     ##   :synthetic, this is a synthetic element - don't emit it's head/tail
                                                     ## ```
  STATE_OWNS_POPUP* = (0x00010000).ELEMENT_STATE_BITS ## ```
                                                      ##   :owns-popup, has visible popup element (tooltip, menu, etc.)
                                                      ## ```
  STATE_TABFOCUS* = (0x00020000).ELEMENT_STATE_BITS ## ```
                                                    ##   :tab-focus, focus gained by tab traversal
                                                    ## ```
  STATE_EMPTY* = (0x00040000).ELEMENT_STATE_BITS ## ```
                                                 ##   :empty, empty - element is empty (text.size() == 0 && subs.size() == 0)
  ...
  STATE_BUSY* = (0x00080000).ELEMENT_STATE_BITS ## ```
                                                ##   :busy, busy doing something (e.g. loading data)
                                                ## ```
  ...
  STATE_POPUP* = (0x08000000).ELEMENT_STATE_BITS ## ```
                                                 ##   :popup, this element is out of flow - shown as popup
                                                 ## ```
  ...
  DRAG_REQUEST* = (0x0000000C).MOUSE_EVENTS ## ```
                                            ...
                                            ##     OBSOLETE
                                            ## ```
  MOUSE_TCLICK* = (0x0000000F).MOUSE_EVENTS ## ```
                                            ##   tripple click
                                            ## ```
  MOUSE_TOUCH_START* = (0x000000FC).MOUSE_EVENTS ## ```
                                                 ##   touch device pressed somehow
                                                 ## ```
  MOUSE_TOUCH_END* = (0x000000FD).MOUSE_EVENTS ## ```
                                               ##   touch device depressed - clear, nothing on it
                                               ## ```
  MOUSE_DRAG_REQUEST* = (0x000000FE).MOUSE_EVENTS ## ```
                                                  ##   mouse drag start detected event
                                                  ## ```
  ...
  DRAGGING* = (0x00000100).MOUSE_EVENTS ## ```
                                        ...
                                        ##     OBSOLETE
                                        ## ```
  MOUSE_HIT_TEST* = (0x00000FFE).MOUSE_EVENTS ## ```
                                              ##   sent to element, allows to handle elements with non-trivial shapes.
                                              ## ```
  ...
  SCROLL_ANIMATION_START* = (SCROLL_SLIDER_PRESSED + 1).SCROLL_EVENTS
  SCROLL_ANIMATION_END* = (SCROLL_ANIMATION_START + 1).SCROLL_EVENTS
  ...
  GESTURE_START* = (0).GESTURE_CMD
  GESTURE_MOVE* = (1).GESTURE_CMD
  GESTURE_END* = (2).GESTURE_CMD
  GESTURE_PAN* = (GESTURE_END + 1).GESTURE_CMD ## ```
                                               ##   The pan gesture.
                                               ## ```
  GESTURE_ZOOM* = (GESTURE_PAN + 1).GESTURE_CMD ## ```
                                                ##   The zoom gesture.
                                                ## ```
  GESTURE_ROTATE* = (GESTURE_ZOOM + 1).GESTURE_CMD ## ```
                                                   ##   The rotation gesture.
                                                   ## ```
  GESTURE_TAP1* = (GESTURE_ROTATE + 1).GESTURE_CMD ## ```
                                                   ##   The tap gesture, a.k.a. click
                                                   ## ```
  GESTURE_TAP2* = (GESTURE_TAP1 + 1).GESTURE_CMD ## ```
                                                 ##   The two-finger tap gesture, a.k.a. right-click
                                                 ## ```
  GESTURE_DOUBLE_TAP* = (GESTURE_TAP2 + 1).GESTURE_CMD ## ```
                                                       ##   The two-finger tap gesture, a.k.a. right-click
                                                       ## ```
  ...
  SELECT_SELECTION_CHANGED* = (5).BEHAVIOR_EVENTS ## ```
                                                  ...
                                                  ## ```
  SELECT_VALUE_CHANGED* = (6).BEHAVIOR_EVENTS ## ```
                                              ##   value of <select> changed
                                              ## ```
  SELECT_STATE_CHANGED* = (SELECT_VALUE_CHANGED).BEHAVIOR_EVENTS ## ```
                                                                 ##   OBSOLETE, alias of SELECT_VALUE_CHANGED
                                                                 ## ```
  ...
  VIDEO_BIND_RQ* = (0x000000D4).BEHAVIOR_EVENTS ## ```
                                                ...
                                                ## ```
  VIDEO_FRAME_REQUEST* = (0x000000D8).BEHAVIOR_EVENTS ## ```
                                                      ##   animation step, a.k.a. animation frame
                                                      ## ```
  ...
  SCITER_SET_UX_THEMING* = (11).SCITER_RT_OPTIONS ## ```
                                                  ##   hWnd = NULL, value - SBOOL, TRUE - the engine will use "unisex" theme that is common for all platforms.
...
type
  ...
  LPUINT* = ptr cuint
  LPCBYTE* = ptr BYTE
  LPCWSTR_RECEIVER* = proc (str: LPCWSTR; str_length: cuint; param: LPVOID): VOID {.
      cdecl.}
  LPCSTR_RECEIVER* = proc (str: LPCSTR; str_length: cuint; param: LPVOID): VOID {.
      cdecl.}
  LPCBYTE_RECEIVER* = proc (str: LPCBYTE; num_bytes: cuint; param: LPVOID): VOID {.
      cdecl.}
  HREQUEST* = pointer ## ```
                      ##   The Sciter Engine of Terra Informatica Software, Inc.
                      ##   
  ...
  SciterRequestAPI* {.bycopy.} = object
    ...
    RequestUrl*: proc (rq: HREQUEST; rcv: LPCSTR_RECEIVER; rcv_param: LPVOID): REQUEST_RESULT {.
        cdecl.}              ## ```
                             ##   get requested URL
                             ## ```
    RequestContentUrl*: proc (rq: HREQUEST; rcv: LPCSTR_RECEIVER;
    ...
    RequestGetRequestedDataType*: proc (rq: HREQUEST;
                                        pData: ptr SciterResourceType): REQUEST_RESULT {.
        cdecl.}              ## ```
                             ##   get requested data type
                             ## ```
    RequestGetReceivedDataType*: proc (rq: HREQUEST; rcv: LPCSTR_RECEIVER;
                                       rcv_param: LPVOID): REQUEST_RESULT {.
        cdecl.}              ## ```
    ...
    RequestGetProxyHost*: proc (rq: HREQUEST; rcv: LPCSTR_RECEIVER;
                                rcv_param: LPVOID): REQUEST_RESULT {.cdecl.} ## ```
                                                                             ##   get proxy host
                                                                             ## ```
    RequestGetProxyPort*: proc (rq: HREQUEST; pPort: ptr cuint): REQUEST_RESULT {.
        cdecl.}              ## ```
                             ##   get proxy port
                             ## ```
    RequestSetSucceeded*: proc (rq: HREQUEST; status: cuint;
                                dataOrNull: LPCBYTE; dataLength: cuint): REQUEST_RESULT {.
        cdecl.}              ## ```
    ...
    RequestSetFailed*: proc (rq: HREQUEST; status: cuint; dataOrNull: LPCBYTE;
                             dataLength: cuint): REQUEST_RESULT {.cdecl.} ## ```
                                                                          ...
                                                                          ## ```
    RequestAppendDataChunk*: proc (rq: HREQUEST; data: LPCBYTE;
                                   dataLength: cuint): REQUEST_RESULT {.cdecl.} ## ```
    ...
    RequestSetReceivedDataEncoding*: proc (rq: HREQUEST; encoding: LPCSTR): REQUEST_RESULT {.
        cdecl.}              ## ```
                             ...
                             ## ```
    RequestGetData*: proc (rq: HREQUEST; rcv: LPCBYTE_RECEIVER;
                           rcv_param: LPVOID): REQUEST_RESULT {.cdecl.} ## ```
  ...
  SciterGraphicsAPI* {.bycopy.} = object
    ...
    RGBA*: proc (red: cuint; green: cuint; blue: cuint; alpha: cuint): SC_COLOR {.
        cdecl.} ## ```
    ...
    gLine*: proc (hgfx: HGFX; x1: SC_POS; y1: SC_POS; x2: SC_POS; y2: SC_POS): GRAPHIN_RESULT {.
        cdecl.} ## ```
                ...
                ## ```
    gRectangle*: proc (hgfx: HGFX; x1: SC_POS; y1: SC_POS; x2: SC_POS;
                       y2: SC_POS): GRAPHIN_RESULT {.cdecl.} ## ```
                                                             ##   Draws rectangle using current lineColor/lineGradient and fillColor/fillGradient with (optional) rounded corners.
                                                             ## ```
    gRoundedRectangle*: proc (hgfx: HGFX; x1: SC_POS; y1: SC_POS; x2: SC_POS;
                              y2: SC_POS; radii8: ptr SC_DIM): GRAPHIN_RESULT {.
        cdecl.} ## ```
    ...
    gEllipse*: proc (hgfx: HGFX; x: SC_POS; y: SC_POS; rx: SC_DIM; ry: SC_DIM): GRAPHIN_RESULT {.
        cdecl.} ## ```
                ...
                ## ```
    gArc*: proc (hgfx: HGFX; x: SC_POS; y: SC_POS; rx: SC_POS; ry: SC_POS;
                 start: SC_ANGLE; sweep: SC_ANGLE): GRAPHIN_RESULT {.cdecl.} ## ```
                                                                             ##   Draws closed arc using current lineColor/lineGradient and fillColor/fillGradient.
                                                                             ## ```
    gStar*: proc (hgfx: HGFX; x: SC_POS; y: SC_POS; r1: SC_DIM; r2: SC_DIM;
                  start: SC_ANGLE; rays: cuint): GRAPHIN_RESULT {.cdecl.} ## ```
                                                                          ##   Draws star.
                                                                          ## ```
    ...
    gDrawPath*: proc (hgfx: HGFX; path: HPATH; dpm: DRAW_PATH_MODE): GRAPHIN_RESULT {.
        cdecl.}
    gRotate*: proc (hgfx: HGFX; radians: SC_ANGLE; cx: ptr SC_POS;
                    cy: ptr SC_POS): GRAPHIN_RESULT {.cdecl.}
    gTranslate*: proc (hgfx: HGFX; cx: SC_POS; cy: SC_POS): GRAPHIN_RESULT {.
        cdecl.}
    ...
    gTransform*: proc (hgfx: HGFX; m11: SC_POS; m12: SC_POS; m21: SC_POS;
                       m22: SC_POS; dx: SC_POS; dy: SC_POS): GRAPHIN_RESULT {.
        cdecl.}              ## ```
    ...
    gLineJoin*: proc (hgfx: HGFX; `type`: SCITER_LINE_JOIN_TYPE): GRAPHIN_RESULT {.
        cdecl.}
    gLineCap*: proc (hgfx: HGFX; `type`: SCITER_LINE_CAP_TYPE): GRAPHIN_RESULT {.
        cdecl.}
    ...
    gLineGradientLinear*: proc (hgfx: HGFX; x1: SC_POS; y1: SC_POS; x2: SC_POS;
                                y2: SC_POS; stops: ptr SC_COLOR_STOP;
    ...
    gFillGradientLinear*: proc (hgfx: HGFX; x1: SC_POS; y1: SC_POS; x2: SC_POS;
                                ...
                                nstops: cuint): GRAPHIN_RESULT {.cdecl.} ## ```
                                                                         ...
                                                                         ## ```
    gLineGradientRadial*: proc (hgfx: HGFX; x: SC_POS; y: SC_POS; rx: SC_DIM;
                                ry: SC_DIM; stops: ptr SC_COLOR_STOP;
                                nstops: cuint): GRAPHIN_RESULT {.cdecl.} ## ```
                                                                         ##   setup parameters of line gradient radial fills.
                                                                         ## ```
    gFillGradientRadial*: proc (hgfx: HGFX; x: SC_POS; y: SC_POS; rx: SC_DIM;
                                ry: SC_DIM; stops: ptr SC_COLOR_STOP;
                                nstops: cuint): GRAPHIN_RESULT {.cdecl.} ## ```
    ...
    textCreateForElementAndStyle*: proc (ptext: ptr HTEXT; text: LPCWSTR;
        textLength: cuint; he: HELEMENT; style: LPCWSTR; styleLength: cuint): GRAPHIN_RESULT {.
        cdecl.} ## ```
    ...
    textGetMetrics*: proc (text: HTEXT; minWidth: ptr SC_DIM;
                           maxWidth: ptr SC_DIM; height: ptr SC_DIM;
                           ascent: ptr SC_DIM; descent: ptr SC_DIM;
                           nLines: ptr cuint): GRAPHIN_RESULT {.cdecl.}
    textSetBox*: proc (text: HTEXT; width: SC_DIM; height: SC_DIM): GRAPHIN_RESULT {.
        cdecl.}
    gDrawText*: proc (hgfx: HGFX; text: HTEXT; px: SC_POS; py: SC_POS;
                      position: cuint): GRAPHIN_RESULT {.cdecl.} ## ```
                                                                 ##   draw text with position (1..9 on MUMPAD) at px,py
                                                                 ##      Ex: gDrawText( 100,100,5) will draw text box with its center at 100,100 px
                                                                 ## ```
    gDrawImage*: proc (hgfx: HGFX; himg: HIMG; x: SC_POS; y: SC_POS;
                       w: ptr SC_DIM; h: ptr SC_DIM; ix: ptr cuint;
                       iy: ptr cuint; iw: ptr cuint; ih: ptr cuint;
                       opacity: ptr cfloat): GRAPHIN_RESULT {.cdecl.} ## ```
    ...
    gScreenToWorld*: proc (hgfx: HGFX; inout_x: ptr SC_POS; inout_y: ptr SC_POS): GRAPHIN_RESULT {.
        cdecl.}
    gPushClipBox*: proc (hgfx: HGFX; x1: SC_POS; y1: SC_POS; x2: SC_POS;
                         y2: SC_POS; opacity: cfloat): GRAPHIN_RESULT {.cdecl.}
    ...
    gPopClip*: proc (hgfx: HGFX): GRAPHIN_RESULT {.cdecl.} ## ```
                                                           ...
                                                           ## ```
    imagePaint*: proc (himg: HIMG; pPainter: ptr image_paint_function;
                       prm: pointer): GRAPHIN_RESULT {.cdecl.} ## ```
    ...
    vUnWrapGfx*: proc (fromValue: ptr VALUE; phgfx: ptr HGFX): GRAPHIN_RESULT {.
        cdecl.}
    vUnWrapImage*: proc (fromValue: ptr VALUE; phimg: ptr HIMG): GRAPHIN_RESULT {.
        cdecl.}
    vUnWrapPath*: proc (fromValue: ptr VALUE; phpath: ptr HPATH): GRAPHIN_RESULT {.
        cdecl.}
    vUnWrapText*: proc (fromValue: ptr VALUE; phtext: ptr HTEXT): GRAPHIN_RESULT {.
        cdecl.}
  ...
  URL_DATA_RECEIVER* = proc (pUrlData: ptr URL_DATA; param: LPVOID): VOID {.
      cdecl.}
  ...
  ISciterAPI* {.bycopy.} = object
    ...
    SciterGetRootElement*: proc (hwnd: ptr GtkWidget; phe: ptr HELEMENT): INT {.
        cdecl.}
    SciterGetFocusElement*: proc (hwnd: ptr GtkWidget; phe: ptr HELEMENT): INT {.
        cdecl.}
    ...
    SciterGetNthChild*: proc (he: HELEMENT; n: cuint; phe: ptr HELEMENT): INT {.
        cdecl.}
    ...
    SciterGetNthAttributeValueCB*: proc (he: HELEMENT; n: cuint;
        rcv: LPCWSTR_RECEIVER; rcv_param: LPVOID): INT {.cdecl.}
    ...
    SciterSetStyleAttribute*: proc (he: HELEMENT; name: LPCSTR; value: LPCWSTR): INT {.
        cdecl.}
    SciterGetElementLocation*: proc (he: HELEMENT; p_location: LPRECT;
    ...
    SciterWindowAttachEventHandler*: proc (hwndLayout: ptr GtkWidget;
        pep: C_LPELEMENT_EVENT_PROC; tag: LPVOID; subscription: cuint): INT {.
        cdecl.}
    ...
    SciterCallBehaviorMethod*: proc (he: HELEMENT; params: ptr METHOD_PARAMS): INT {.
        cdecl.}
    SciterRequestElementData*: proc (he: HELEMENT; url: LPCWSTR;
                                     dataType: cuint; initiator: HELEMENT): INT {.
        cdecl.}
    SciterHttpRequest*: proc (he: HELEMENT; url: LPCWSTR; dataType: cuint;
                              requestType: cuint;
                              requestParams: ptr REQUEST_PARAM; nParams: cuint): INT {.
        cdecl.}
    ...
    SciterCallScriptingFunction*: proc (he: HELEMENT; name: LPCSTR;
                                        argv: ptr VALUE; argc: cuint;
                                        retval: ptr VALUE): INT {.cdecl.}
    SciterEvalElementScript*: proc (he: HELEMENT; script: LPCWSTR;
                                    scriptLength: cuint; retval: ptr VALUE): INT {.
        cdecl.}
    SciterAttachHwndToElement*: proc (he: HELEMENT; hwnd: ptr GtkWidget): INT {.
        cdecl.}
    ...
    SciterGetHighlightedElement*: proc (hwnd: ptr GtkWidget; phe: ptr HELEMENT): INT {.
        cdecl.}
    SciterSetHighlightedElement*: proc (hwnd: ptr GtkWidget; he: HELEMENT): INT {.
        cdecl.}              ## ```
                             ##   |
                             ##     | DOM Node API
                             ##     |
    ...
    SciterNodeNthChild*: proc (hnode: HNODE; n: cuint; phn: ptr HNODE): INT {.
        cdecl.}
    ...
    ValueType*: proc (pval: ptr VALUE; pType: ptr cuint; pUnits: ptr cuint): cuint {.
        cdecl.}
    ValueStringData*: proc (pval: ptr VALUE; pChars: ptr LPCWSTR;
    ...
    ValueIntDataSet*: proc (pval: ptr VALUE; data: INT; `type`: cuint;
                            units: cuint): cuint {.cdecl.}
    ValueInt64Data*: proc (pval: ptr VALUE; pData: ptr INT64): cuint {.cdecl.}
    ValueInt64DataSet*: proc (pval: ptr VALUE; data: INT64; `type`: cuint;
                              units: cuint): cuint {.cdecl.}
    ValueFloatData*: proc (pval: ptr VALUE; pData: ptr cdouble): cuint {.cdecl.}
    ValueFloatDataSet*: proc (pval: ptr VALUE; data: cdouble; `type`: cuint;
                              units: cuint): cuint {.cdecl.}
    ValueBinaryData*: proc (pval: ptr VALUE; pBytes: ptr LPCBYTE;
                            pnBytes: ptr cuint): cuint {.cdecl.}
    ...
    ValueNthElementValue*: proc (pval: ptr VALUE; n: INT; pretval: ptr VALUE): cuint {.
        cdecl.}
    ValueNthElementValueSet*: proc (pval: ptr VALUE; n: INT;
                                    pval_to_set: ptr VALUE): cuint {.cdecl.}
    ValueNthElementKey*: proc (pval: ptr VALUE; n: INT; pretval: ptr VALUE): cuint {.
        cdecl.}
    ...
    ValueGetValueOfKey*: proc (pval: ptr VALUE; pkey: ptr VALUE;
                               pretval: ptr VALUE): cuint {.cdecl.}
    ValueToString*: proc (pval: ptr VALUE; how: cuint): cuint {.cdecl.}
    ValueFromString*: proc (pval: ptr VALUE; str: LPCWSTR; strLength: cuint;
                            how: cuint): cuint {.cdecl.}
    ValueInvoke*: proc (pval: ptr VALUE; pthis: ptr VALUE; argc: cuint;
                        argv: ptr VALUE; pretval: ptr VALUE; url: LPCWSTR): cuint {.
        cdecl.}
    ...
    SciterSetVariable*: proc (hwndOrNull: ptr GtkWidget; path: LPCWSTR;
                              pvalToSet: ptr VALUE): cuint {.cdecl.}
    SciterGetVariable*: proc (hwndOrNull: ptr GtkWidget; path: LPCWSTR;
                              pvalToGet: ptr VALUE): cuint {.cdecl.}
    SciterElementUnwrap*: proc (pval: ptr VALUE; ppElement: ptr HELEMENT): cuint {.
        cdecl.}
    SciterElementWrap*: proc (pval: ptr VALUE; pElement: HELEMENT): cuint {.
        cdecl.}
    SciterNodeUnwrap*: proc (pval: ptr VALUE; ppNode: ptr HNODE): cuint {.cdecl.}
    SciterNodeWrap*: proc (pval: ptr VALUE; pNode: HNODE): cuint {.cdecl.}
...
proc ValueInt64Data*(pval: ptr VALUE; pData: ptr INT64): cuint {.importc, cdecl.}
  ## ```
                                                                                 ...
                                                                                 ## ```
proc ValueInt64DataSet*(pval: ptr VALUE; data: INT64; `type`: cuint;
                        units: cuint): cuint {.importc, cdecl.}
  ## ```
                                                               ##   ValueInt64DataSet - sets 64bit integer data of T_CURRENCY and T_DATE values.
                                                               ## ```
proc ValueFloatData*(pval: ptr VALUE; pData: ptr cdouble): cuint {.importc,
    cdecl.}
  ## ```
           ##   ValueFloatData - retreive FLOAT_VALUE (double) data of T_FLOAT and T_LENGTH values.
           ## ```
proc ValueFloatDataSet*(pval: ptr VALUE; data: cdouble; `type`: cuint;
                        units: cuint): cuint {.importc, cdecl.}
  ## ```
...
proc ValueBinaryData*(pval: ptr VALUE; pBytes: ptr LPCBYTE; pnBytes: ptr cuint): cuint {.
  ...
  ## ```
                    ...
                    ## ```
proc ValueBinaryDataSet*(pval: ptr VALUE; pBytes: LPCBYTE; nBytes: cuint;
...
proc ValueEnumElements*(pval: ptr VALUE; penum: KeyValueCallback;
                        param: LPVOID): cuint {.importc, cdecl.}
...
proc SciterGetRootElement*(hwnd: ptr GtkWidget; phe: ptr HELEMENT): INT {.
  ...
  ## ```
                    ...
                    ## ```
proc SciterGetFocusElement*(hwnd: ptr GtkWidget; phe: ptr HELEMENT): INT {.
    importc, cdecl.}
  ## ```
                    ...
                    ## ```
proc SciterFindElement*(hwnd: ptr GtkWidget; pt: POINT; phe: ptr HELEMENT): INT {.
    importc, cdecl.}
  ## ```
                    ...
                    ## ```
proc SciterGetChildrenCount*(he: HELEMENT; count: ptr cuint): INT {.importc,
    cdecl.}
  ## ```
...
proc SciterGetElementLocation*(he: HELEMENT; p_location: LPRECT; areas: cuint): INT {.
    importc, cdecl.}
proc SciterScrollToView*(he: HELEMENT; SciterScrollFlags: cuint): INT {.importc,
    cdecl.}
  ## ```
           ##   Scroll to view.
           ##   
...
proc SciterEvalElementScript*(he: HELEMENT; script: LPCWSTR;
                              scriptLength: cuint; retval: ptr VALUE): INT {.
    importc, cdecl.}
proc SciterAttachHwndToElement*(he: HELEMENT; hwnd: ptr GtkWidget): INT {.
    importc, cdecl.}
  ## ```
...
proc SciterGetHighlightedElement*(hwnd: ptr GtkWidget; phe: ptr HELEMENT): INT {.
  ...
  ## ```
                    ...
                    ## ```
proc SciterSetHighlightedElement*(hwnd: ptr GtkWidget; he: HELEMENT): INT {.
    importc, cdecl.}
proc SciterNodeAddRef*(hn: HNODE): INT {.importc, cdecl.}
proc SciterNodeRelease*(hn: HNODE): INT {.importc, cdecl.}
proc SciterNodeCastFromElement*(he: HELEMENT; phn: ptr HNODE): INT {.importc,
    cdecl.}
...
proc SciterNodeParent*(hnode: HNODE; pheParent: ptr HELEMENT): INT {.importc,
    cdecl.}
proc SciterNodeNthChild*(hnode: HNODE; n: cuint; phn: ptr HNODE): INT {.importc,
    cdecl.}
...
proc SciterNodeGetText*(hnode: HNODE; rcv: LPCWSTR_RECEIVER;
                        rcv_param: LPVOID): INT {.importc, cdecl.}
proc SciterNodeSetText*(hnode: HNODE; text: LPCWSTR; textLength: cuint): INT {.
    importc, cdecl.}
proc SciterNodeInsert*(hnode: HNODE; where: cuint; what: HNODE): INT {.importc,
    cdecl.}
  ## ```
...
proc SciterCreateTextNode*(text: LPCWSTR; textLength: cuint; phnode: ptr HNODE): INT {.
  ...
  ## ```
                    ...
                    ## ```
proc SciterCreateCommentNode*(text: LPCWSTR; textLength: cuint;
